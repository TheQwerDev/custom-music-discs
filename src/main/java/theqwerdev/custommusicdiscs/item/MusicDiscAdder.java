...
public class MusicDiscAdder {
	...
	private static final ArrayList<Item> discs = new ArrayList<>();
	public static Map<Integer, File> trackMap;

	public static void getTrackMap() {
		...
		for (File track : trackMap.values()) {
			...
			for (File data : trackData) {
				...
				if (!foundTextureFile && name.equals("texture.png")) {
					...
					TexturePackGenerator.addDiscTexture(data, trackNumber);
